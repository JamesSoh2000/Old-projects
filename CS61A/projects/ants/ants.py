...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        Asks the insect to remove itself from the current place. This method exists so
            it can be enhanced in subclasses.
        """
        # You can't remove the real Queen in the game, except any impostor.
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if self.can_contain(place.ant): #isinstance(self, BodyguardAnt) and
                self.contain_ant(place.ant)
                place.ant = self
            elif place.ant.can_contain(self): #isinstance(place.ant, BodyguardAnt)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        ant_location = self.place
        space = 0

        while ant_location != beehive:
            if ant_location.bees and space >= self.min_range and space <= self.max_range:
                return random_or_none(ant_location.bees)  # REPLACE THIS LINE
            space += 1
            ant_location = ant_location.entrance
        return None
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        # 이런식으로 QueenAnt class 자체를 modify할수 있음.
    ...
    def action(self, gamestate):
        ...
        if not self.impostor:
            ...
            while behind is not None:
                ...
                if behind.ant is not None and behind.ant.buff_status == False:
                    behind.ant.buff_status = True
                    behind.ant.damage *= 2
                # There is both container ant and contained_ant.
                if isinstance(behind.ant, ContainerAnt):
                    if behind.ant.contained_ant and not behind.ant.contained_ant.buff_status:
                        behind.ant.contained_ant.buff_status = True
