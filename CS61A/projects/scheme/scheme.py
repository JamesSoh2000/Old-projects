...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # 여기서 예로 우리의 expr = read_line('(+ 2 2)') => Pair('+', Pair(2, Pair(2, nil))) 라고 가정
        # Evaluate the operator(예를 들어 +나 -). 여기선 +.
        procedure = scheme_eval(first, env)
        # Now Check if this operator is a Scheme procedure
        validate_procedure(procedure)

        # Now we evaluate the args. 여기선 Pair(2, Pair(2, nil))
        args = rest.map(lambda x: scheme_eval(x, env))
        # 그리고 scheme_apply로 +(procedure) 를
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        # 예를 들어보죠
        # expression = read_line("(x (+ 2 8))")
        # (x (+ 2 8)) -> Pair('x', Pair(Pair('+', Pair(2, Pair(8, nil))), nil))
        # expression.first -> 'x'
        # expression.rest -> Pair(Pair('+', Pair(2, Pair(8, nil))), nil)
        # expression.rest.first -> Pair('+', Pair(2, Pair(8, nil)))
        ...
        # expression = read_line("((f x) (+ 2 8))") 이면 scheme에서는 (define (f x) (+ 2 8)) 로 표현된다.
        # 이것을 (define f (lambda (x) (+ 2 8))) 로 바꿔주는게 여기서 관건임.
        # 먼저 위에 isinstance(target, Pair)로 expressions.first(target)이 (f x)와 같은 Pair인지 확인한다.
        # 확인이 되면 function의 이름인 'f'를 가져와주기위해 target.first로 저장
        f_name = target.first
        # 그 후에는 lambda expression으로 바꿔주기위해
        f_body = do_lambda_form(Pair(target.rest, expressions.rest), env)
...
def do_quote_form(expressions, env):
    ...
    """
    # -> do_quote_form을 하기 전 모습은 Pair(Pair('+', Pair('x', Pair(2, nil))))
