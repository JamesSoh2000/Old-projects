...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        for (int column = 0; column < board.size(); column ++) {
            for (int row = board.size() - 1; row > 0; row --) {
                Tile current_tile = board.tile(column, row);
                Tile belowTile = lookTileBelow(column, row - 1, board);
                if (current_tile == null) {

                    if (belowTile != null) {
                        board.move(column, row, belowTile);
                        row += 1;
                        changed = true;
                    }
                ...
                // Current tile is not null.
                    ...
                    if (belowTile != null) {
                        if (current_tile.value() == belowTile.value()) {
                            board.move(column, row, belowTile);
                            score += board.tile(column, row).value();
                        } else {
                            board.move(column, row - 1, belowTile);
                        }
                        changed = true;
                    }
    ...
    public static Tile lookTileBelow(int col, int startRow, Board board) {
        // Checking all rows until we find an existing tile, if not, we just return null.
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        //         if ((-1 < (col - pointerA)) && (4 > (pointerA + col)) && (4 > (pointerA + row)) && (-1 < (row - pointerA))) {
        //             if ((b.tile(col + pointerA, row).value() == value) || (b.tile(col - pointerA, row).value() == value)) {
        //                 return true;
        //             } else if (b.tile(col, row + pointerA).value() == value || b.tile(col, row - pointerA).value() == value) {
        //                 return true;
        //             }
        //         }
        //     }
